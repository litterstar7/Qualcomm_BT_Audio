...
typedef enum apps_slt_id
{
    ...
    /*! Pointer to on-chip coredump region table location.
     * The SLT entry points to another pointer.
     * That pointer references a table of "interesting" data that the
     * on-chip coredump feature will try to save in the event of a panic.
     */
    APPS_SLT_ID_COREDUMP_REGION_PTR = 14
