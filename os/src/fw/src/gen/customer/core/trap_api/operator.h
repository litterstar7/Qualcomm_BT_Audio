...
/**
 *  \brief Load configuration data file for an operator.
 * This loads operator configuration data from an address
 * range in a RA partition into the audio-DSP operator framework. This should be
 * done before calling OperatorStart() so that the operator capability can use the
 * configuration data during its operation. The application should call
 * OperatorMessage() to set up the operator to use the downloaded operator data.
 * The downloaded data will occupy DSP resources until unloaded with
 * OperatorDataUnloadRa(), or until the audio-DSP is powered off by calling
 * OperatorFrameworkEnable(MAIN_PROCESSOR_OFF). However, the application can
 * request the operator framework to unload the operator data on calling
 ...
 * framework before starting SVA capability. 
 *           
 *  \param handle Pointer to the handle of the partition containing operator data
 *  \param offset Offset into the partition of the operator data, in bytes
 *  \param length Length of the operator data, in bytes
 *  \param type Type of operator data file
 *  \param storage Type of storage used by audio-DSP framework for storing operator data 
 *  \param auto_remove TRUE if operator data needs to be removed on operator destroy, FALSE othewise
 *  \return A handle for the resources occupied by the loaded data file which can be passed
 *  to OperatorDataUnloadRa() 
 * 
 * \ingroup trapset_operator
 */
