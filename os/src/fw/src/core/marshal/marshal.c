...
static bool write_uint8(marshal_base_t *base, uint8 value)
{
...
static bool write_pointer_index(marshal_base_t *base, mob_t *referred)
{
    mob_index_t referred_index;

    referred->address = dereference_pp(referred->address);
...
#else
    assert(mobs_has_object(&base->object_set, referred, &referred_index));
#endif

    return write_uint8(base, referred_index);
}

static bool write_value(marshal_base_t *base, void *leaf_address, const marshal_type_descriptor_t *type_desc)
{
    marshal_context_t *m = STRUCT_FROM_MEMBER(marshal_context_t, base, base);
    ...
    if (m->buf)
    {
        ...
        if (next_index <= m->size)
        {
            ...
            (void)copy_cb(m->buf + m->index, leaf_address, type_desc->size);
            m->index = next_index;
            return TRUE;
        }
    }
                             ...
                             mob_t *parent)
{
    ...
    /* Whilst traversing to register pointed objects, also register the
       pointer members to avoid a subsequent traverse */
    referred.address = pointer_addr;
    referred.disambiguator = 0;
    mobs_push_unique(&base->pointer_member_set, &referred);
...
/* Write the index of pointer members to the marshal byte stream */
static bool marshal_pointer_indexes(mobs_t *set, mob_index_t index, mob_t *object)
{
    ...
    if (write_pointer_index(base, object))
    {
...
static bool count_remaining_value(marshal_base_t *base, void *leaf_address, const marshal_type_descriptor_t *type_desc)
{
    marshal_context_t *m = STRUCT_FROM_MEMBER(marshal_context_t, base, base);
    m->remaining += type_desc->size;
    UNUSED(leaf_address);
    return TRUE;
}
...
bool marshal(marshal_context_t *m, void *addr, marshal_type_t type)
{
    ...
    switch (m->state)
    {
        ...
        {
            if (mobs_iterate(&m->base.pointer_member_set, m->next_pointers_index, marshal_pointer_indexes))
            {
                m->state = REGISTER_POINTERS;
                m->next_pointers_index = 0;
                base_reset_pointer_member_set(&m->base);
                return TRUE;
            }
        }
        break;
...
size_t marshal_remaining(marshaller_t m)
{
    ...
    switch (m->state)
    {
        ...
        /*lint -fallthrough */
        case MARSHAL_POINTER_INDEXES:
            m->remaining += (mobs_size(&m->base.pointer_member_set) - m->next_pointers_index);
        break;
    }
    return m->remaining;
}
