...
bool appChargerCanPowerOff(void)
{
    ...
    else
    {
        ...
        DEBUG_LOG("appChargerCanPowerOff status 0x%x detection pending 0x%x", ChargerStatus(), appChargerDetectionIsPending());
...
bool appChargerInit(Task init_task)
{
    ...
    theCharger->force_allow_power_off = FALSE;
...
void appChargerForceAllowPowerOff(bool force_allow_power_off)
{
    chargerTaskData *theCharger = appGetCharger();
    theCharger->force_allow_power_off = force_allow_power_off;
}
