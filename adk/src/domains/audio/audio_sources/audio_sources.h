...
/*! \brief Audio Sources UI Provider contexts
    \note This enumerated type must be maintained in order of ascending priority.
          An Audio Source which has been disconnected has the lowest prioirity in
          audio routing, whereas an audio source that is streaming has the highest
          priority. */
typedef enum
{
    context_audio_disconnected,
    context_audio_connected,
    context_audio_is_streaming
...
/*! \brief Inform source of it's current routing state
    ...
    \return audio_source_status_ready if audio router can continue,
     audio_source_status_preparing if the audio router should wait.
 */
audio_source_status_t AudioSources_SetState(audio_source_t source, audio_source_state_t state);
...
/*! \brief Calls the OnAudioRouted observer function of a sources registered observer interface.
    ...
    \param source The audio source
 */
void AudioSources_OnAudioRouted(audio_source_t source);
