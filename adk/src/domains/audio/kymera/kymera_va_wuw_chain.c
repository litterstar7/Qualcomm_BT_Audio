...
static void kymera_ConfigureWuwEngine(Operator wuw, const void *params)
{
    ...
    {
        wuw_model_handle = PanicZero(VoiceUi_LoadHotwordModelFile(op_params->wuw_model));
    }
...
void Kymera_GetWuwEngineVersion(va_audio_wuw_engine_version_t *version)
{
    va_wuw_chain_params_t chain_params = { .wuw_engine = va_wuw_engine_gva };
    ...
    {
        kymera_CreateChain(&chain_params);
    }

    Operator op = kymera_GetChainOperator(OPR_WUW);
