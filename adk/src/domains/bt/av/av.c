...
bool Av_InstanceIsStarted(avInstanceTaskData* av_instance)
{
...
*/
bool Av_InstanceIsConnected(avInstanceTaskData* theInst)
{
    if (theInst && appA2dpIsSinkCodec(theInst))
    {
        return TRUE;
...
static void av_StoreHandsetVolumeDeviceProperty(avInstanceTaskData *theInst)
{
    ...
    DEBUG_LOG("av_StoreHandsetVolumeDeviceProperty addr=%x,%x,%x vol=%u volume_set=%d",
                bd_addr.nap, bd_addr.uap, bd_addr.lap, theInst->volume, volume_set);
}
...
static bool av_CheckAndSetHandsetVolumeIfNeeded(avInstanceTaskData *theInst)
{
    ...
    if (appDeviceIsHandset(&theInst->bd_addr))
    {
        ...
        if ((theInst->volume) == VOLUME_UNSET)
        {
            ...
            Volume_SendAudioSourceVolumeUpdateRequest(audio_source_a2dp_1, av_GetOrigin(theInst), volume);
...
*/
static void av_VolumeHandleAvrcpConnect(avInstanceTaskData *theInst)
{
    bool instance_device_is_handset = av_CheckAndSetHandsetVolumeIfNeeded(theInst);

    if (!instance_device_is_handset && appDeviceIsPeer(&theInst->bd_addr))
    {
        ...
        if (appDeviceIsHandsetA2dpConnected())
        {
            ...
            if  (appDeviceGetHandsetBdAddr(&handset_bd_addr) && !appDeviceIsTwsPlusHandset(&handset_bd_addr))
            {
                ...
                if (handsetInst)
                {
                    ...
                    DEBUG_LOG("av_VolumeHandleAvrcpConnect(%p), peer connected, handset already connected, volume=%u", theInst, volume);

                    /* Pass in handset instance so we don't attempt to send volume to handset */
                    Volume_SendAudioSourceVolumeUpdateRequest(audio_source_a2dp_1,  av_GetOrigin(handsetInst), volume);
                }
            }
        }
    }
...
static void appAvInstanceHandleAvAvrcpConnectCfm(avInstanceTaskData *theInst, AV_AVRCP_CONNECT_CFM_T *cfm)
{
    ...
    if (cfm->status == avrcp_success)
    {
        ...
        av_VolumeHandleAvrcpConnect(theInst);
...
static void appAvA2dpSyncDisable(avInstanceTaskData *theInst)
{
    ...
    {
        DEBUG_LOG_INFO("appAvA2dpSyncDisable Nothing to do inst:%p other_inst:%p", theInst, theOtherInst);
    }
}
...
static bool audio_source_a2dp_1_used_for_handset = FALSE;

static void av_SearchForHandsetWithAudioSourceA2DP_1(device_t device, void * data)
{
    ...
    {
        if ((*ptr_to_source == audio_source_a2dp_1) &&
            (BtDevice_GetDeviceType(device) == DEVICE_TYPE_HANDSET))
        {
            audio_source_a2dp_1_used_for_handset = TRUE;
        }
    }
}
...
{
    audio_source_t source = audio_source_a2dp_1;
    device_t source_device = NULL;
    device_t device = Av_FindDeviceFromInstance(theInst);
    PanicFalse(device != NULL);

    /* Find a free audio source */
    audio_source_a2dp_1_used_for_handset = FALSE;
    DeviceList_Iterate(av_SearchForHandsetWithAudioSourceA2DP_1, NULL);
    if (audio_source_a2dp_1_used_for_handset)
    {
        /* If a2dp_1 has been allocated, try to allocate a2dp_2 */
        source = audio_source_a2dp_2;
        source_device = DeviceList_GetFirstDeviceWithPropertyValue(
                    device_property_audio_source, &source, sizeof(audio_source_t));
    }
    if (source_device == NULL)
    {
        ...
        DEBUG_LOG_VERBOSE("Av_AllocateAudioSourceToDevice inst=%08x source=%d", theInst, source);
    }
    else
    {
...
static void av_DeallocateAudioSourceFromDevice(device_t device)
{
...
audio_source_provider_context_t AvGetCurrentContext(audio_source_t source)
{
    ...
    if(av_instance)
    {
        ...
        {
            if(Av_InstanceIsStarted(av_instance))
            {
                context = context_audio_is_streaming;
            }
            else if (Av_InstanceIsConnected(av_instance))
            {
...
static void av_RegisterMediaControlInterfaceForInstance(avInstanceTaskData* av_inst)
{
...
avInstanceTaskData *appAvInstanceCreate(const bdaddr *bd_addr)
{
    ...
    avInstanceTaskData* av_inst = AvInstance_GetInstanceForDevice(device);
    ...
    {
        /* Aura 2.0 and Aura 2.1 shall always use A2DP 1 audio source for peer forwarding. */
        audio_source_t source = audio_source_a2dp_1;
        Device_SetProperty(device, device_property_audio_source, &source, sizeof(audio_source_t));
    }
    else
    ...
    av_RegisterMediaControlInterfaceForInstance(av_inst);
...
static bool appAvA2dpLockPending(void)
{
    ...
    for_all_av_instances(theInst, &iterator)
    {
        ...
        {
            DEBUG_LOG("appAvA2dpLockPending %p %d",
                &appA2dpGetLock(theInst), appA2dpGetLock(theInst));
...
void appAvInstanceDestroy(avInstanceTaskData *theInst)
{
    ...
    if (appA2dpIsDisconnected(theInst) && appAvrcpIsDisconnected(theInst) && !appAvA2dpLockPending())
    {
        ...
        av_DeallocateAudioSourceFromDevice(device);
...
static void appAvEnterInitialisingA2dp(avTaskData *theAv)
{
    ...
    sep_data_type seps[] = {
        ...
        { .sep_config = &av_aptxhd_snk_sep,
          .in_use = (FeatureVerifyLicense(APTX_CLASSIC) && appConfigAptxHdEnabled()) ? 0 : A2DP_SEP_UNAVAILABLE,
        },
        ...
        { .sep_config = &av_aptx_adaptive_tws_snk_sep,
          .in_use = FeatureVerifyLicense(APTX_ADAPTIVE_MONO_DECODE) && appConfigAptxAdaptiveEnabled() ? 0 : A2DP_SEP_UNAVAILABLE,
        },
#endif
        /* TWS sources */
...
avrcp_play_status appAvPlayStatus(void)
{
    ...
    for_all_av_instances(theInst, &iterator)
    {
        ...
        {
            return Av_InstancePlayStatus(theInst);
        }
    }
...
avrcp_play_status Av_InstancePlayStatus(avInstanceTaskData *av_instance)
{
