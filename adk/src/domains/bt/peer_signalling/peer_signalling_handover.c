...
static bool peerSignalling_Veto(void)
{
    ...
    {
        DEBUG_LOG_INFO("peerSignalling_Veto, Messages pending for Peer signalling task");
        veto = TRUE;
    }
    else
    {
        ...
        {
            DEBUG_LOG_INFO("peerSignalling_Veto, Not in connected state(%d)",the_inst->state);
            veto = TRUE;
        }
        else if (TaskList_Size(&the_inst->disconnect_tasks))
        {
            DEBUG_LOG_INFO("peerSignalling_Veto, Client task has initiated disconnect of peer-signalling");
            veto = TRUE;
        }
        else if (appPeerSigCheckForPendingMarshalledMsg())
        {
            DEBUG_LOG_INFO("peerSignalling_Veto, Pending messages");
            veto = TRUE;
        }
