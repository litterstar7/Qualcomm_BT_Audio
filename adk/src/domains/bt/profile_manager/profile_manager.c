...
static void profileManager_HandleConnectedProfileInd(bdaddr* bd_addr, uint32 profile, profile_request_status_t status)
{
...
static void profileManager_HandleDisconnectedProfileInd(bdaddr* bd_addr, uint32 profile, profile_request_status_t status, profile_manager_disconnected_ind_reason_t reason)
{
    device_t device = BtDevice_GetDeviceForBdAddr(bd_addr);
...
static void profileManager_HandleMessage(Task task, MessageId id, Message message)
{
    ...
    DEBUG_LOG("profileManager_HandleMessage id=0x%04X", id);
...
{
    DEBUG_LOG_DEBUG("ProfileManager_RegisterProfile: profile=%d", profile);
...
{
    DEBUG_LOG("ProfileManager_ConnectProfilesRequest(%p, %p)", client, device);
...
{
    DEBUG_LOG("ProfileManager_DisconnectProfilesRequest(%p, %p)", client, device);
...
void ProfileManager_GenericConnectedInd(profile_t profile, bdaddr *bd_addr)
{
    DEBUG_LOG("ProfileManager_GenericConnectedInd(%d, [%lx, %x, %x])",
...
void ProfileManager_GenericDisconnectedInd(profile_t profile, bdaddr *bd_addr, uint8 reason)
{
    DEBUG_LOG("ProfileManager_GenericDisconnectedInd(%d, [%lx, %x, %x], %d)",
    ...
    if (profile < profile_manager_max_number_of_profiles)
        profileManager_HandleDisconnectedProfileInd(bd_addr, profileManager_ConvertToBtProfile(profile), 
            request_succeeded, profile_manager_disconnected_normal);
     else
...
{
    DEBUG_LOG("ProfileManager_GenericConnectCfm(%d, %p, %d)", profile,  device, successful);

    if (profile < profile_manager_max_number_of_profiles)
         profileManager_HandleProfileStatusChange(device, profileManager_ConvertToBtProfile(profile),
...
{
    DEBUG_LOG("ProfileManager_GenericDisconnectCfm(%d, %p, %d)", profile,  device, successful);
