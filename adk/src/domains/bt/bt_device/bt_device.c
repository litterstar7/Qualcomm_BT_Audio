...
static void btDevice_PrintDeviceInfo(device_t device, void *data)
{
    ...
    if (Device_GetPropertyU8(device, device_property_a2dp_volume, (void *)&volume))
    {
    ...
    hfp_profile peer_hfp_profile_connected)
{
    ...
    if (device)
    {
        ...
        DEBUG_LOG("SetProfileConnectedAndSupportedFlagsFromPeer, connected=0x%08x, supported=0x%08x",
...
void BtDevice_GetDeviceData(device_t device, bt_device_pdd_t *device_data)
{
    ...
    Device_GetPropertyU8(device, device_property_reserved_1, &device_data->reserved_1);
    Device_GetPropertyU8(device, device_property_reserved_2, &device_data->reserved_2);
    Device_GetPropertyU32(device, device_property_supported_profiles, &device_data->supported_profiles);
    Device_GetPropertyU32(device, device_property_last_connected_profiles, &device_data->last_connected_profiles);

    /* Cater for an upgrade from 8-bit to 32-bit profiles. */
    if ((device_data->reserved_1 != 0) && (device_data->supported_profiles == 0))
    {
        device_data->supported_profiles = (uint32) device_data->reserved_1;
        Device_SetPropertyU32(device, device_property_supported_profiles, device_data->supported_profiles);
        Device_SetPropertyU8(device, device_property_reserved_1, 0);
    }
    if ((device_data->reserved_2 != 0) && (device_data->last_connected_profiles == 0))
    {
        device_data->last_connected_profiles = (uint32) device_data->reserved_2;
        Device_SetPropertyU32(device, device_property_last_connected_profiles, device_data->last_connected_profiles);
        Device_SetPropertyU8(device, device_property_reserved_2, 0);
    }
