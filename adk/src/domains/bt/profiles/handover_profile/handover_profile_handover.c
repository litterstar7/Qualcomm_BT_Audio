...
static handover_profile_status_t handoverProfile_SendHandoverMsg(void *src_addr, uint16 size, uint32 timeout)
{
    ...
    if(dst_addr)
    {
        ...
        {
            DEBUG_LOG_INFO("handoverProfile_SendHandoverMsg: SinkFlush() failed");
...
static bool handoverProfile_SecondaryVeto(const bdaddr *addr, uint8 pri_tx_seq, uint8 pri_rx_seq, uint16 mirror_state)
{
    ...
    else if(appAvInstanceFindFromBdAddr(addr))
    {
        ...
        DEBUG_LOG_INFO("handoverProfile_SecondaryVeto: AV instance exist!");
        return TRUE;
    }
...
/*! \brief Handle the version indication message from the secondary and determine
           if handover is allowed
*/
static void handoverProfile_ProcessVersionInd(const uint8 *data, uint16 size)
{
    handover_profile_task_data_t *ho_inst = Handover_GetTaskData();
    HANDOVER_PROFILE_VERSION_IND_T ind;

    if(ho_inst->is_primary &&
       ho_inst->state == HANDOVER_PROFILE_STATE_CONNECTED &&
       size == sizeof(HANDOVER_PROFILE_VERSION_IND_T))
    {
        uint32 appsp0_version = VmGetFwVersion(FIRMWARE_ID);
        uint32 appsp1_version = VmGetFwVersion(APPLICATION_ID);

        memmove(&ind, data, size);

        DEBUG_LOG("handoverProfile_ProcessVersionInd sec: 0x%x 0x%x 0x%x 0x%x",
                    ind.appsp0_version, ind.appsp1_version, ind.btss_rom_version, ind.btss_patch_version);
        DEBUG_LOG("handoverProfile_ProcessVersionInd pri: 0x%x 0x%x 0x%x 0x%x",
        ...
        {
            DEBUG_LOG("handoverProfile_ProcessVersionInd firmware is matched, handover will be allowed");
            ho_inst->secondary_firmware = HANDOVER_PROFILE_SECONDARY_FIRMWARE_MATCHED;
        }
        else
        {
            DEBUG_LOG("handoverProfile_ProcessVersionInd firmware is mismatched, handover will not be allowed");
            ho_inst->secondary_firmware = HANDOVER_PROFILE_SECONDARY_FIRMWARE_MISMATCHED;
        }
    }
    else
    {
        DEBUG_LOG("handoverProfile_ProcessVersionInd failed, msg size=%d", size);
    }
    ...
    uint16 size, uint16 *consumed)
{
    ...
    {
        DEBUG_LOG_INFO("handoverProfile_ProcessProtocolStartReq failed, ho_inst->is_primary0x%x, ho_inst->state=0x%x, msg size=%d",
    ...
    *consumed = size;
    ...
    if (handoverProfile_SecondaryVeto(&req.handset_addr, req.last_tx_seq, req.last_rx_seq, req.mirror_state))
    {
        ...
        {
            DEBUG_LOG_INFO("handoverProfile_ProcessProtocolStartReq failed to send handover start cfm");
        }
        return FALSE;
    }
...
bool HandoverProfile_VetoP1Clients(void)
{
    ...
    for(uint8 client_index=0; client_index < ho_inst->num_clients; client_index++)
    {
        ...
        {
            DEBUG_LOG_INFO("HandoverProfile_VetoP1Clients. handover Client (%d) Veto'ed", client_index);
            return TRUE;
        }
    }
    return FALSE;
    ...
    uint32 timeout)
{
    ...
    if(dest == NULL)
    {
        DEBUG_LOG_INFO("handoverProfile_ClaimSink SinkMap returned NULL");
        return NULL;
    }
    ...
    if(timedout)
    {
        DEBUG_LOG_INFO("handoverProfile_ClaimSink timedout during attempt to claim %u bytes", size-already_claimed);
        return NULL;
    }
    ...
    if(offset == HANDOVER_PROFILE_INVALID_SINK_CLAIM_SIZE)
    {
        DEBUG_LOG_INFO("handoverProfile_ClaimSink SinkClaim returned Invalid Offset");
        return NULL;
    }
    ...
    uint16 src_size, uint16 *consumed)
{
       ...
       ((p0_data_size = CONVERT_TO_UINT16(&src_addr[HANDOVER_PROFILE_P0_DATA_LEN_OFFSET])) != 0))
    {
        ...
        *consumed = *consumed + p0_data_size;
        ...
        if(src_size > (p0_data_size + HANDOVER_PROFILE_P0_HEADER_LEN))
        {
            ...
            {
                DEBUG_LOG_INFO("handoverProfile_ProcessMarshalData, Unknown data received");
            }
        }
        HandoverPioClr();
    }
    else
    {
        ...
        if(ret)
        {
            ...
            {
                DEBUG_LOG_INFO("handoverProfile_ProcessMarshalData, Failed to send HANDOVER_PROTOCOL_UNMARSHAL_P1_CFM");
                HandoverProfile_AbortP1Clients();
            }
...
handover_profile_status_t HandoverProfile_SendHandoverVersionInd(void)
{
    handover_profile_task_data_t *ho_inst = Handover_GetTaskData();
    HANDOVER_PROFILE_VERSION_IND_T ind;

    memset(&ind, 0, sizeof(HANDOVER_PROFILE_VERSION_IND_T));
    ind.opcode = HANDOVER_PROFILE_VERSION_IND;
    ind.appsp0_version = VmGetFwVersion(FIRMWARE_ID);
    ind.appsp1_version = VmGetFwVersion(APPLICATION_ID);
    ind.btss_rom_version = ho_inst->btss_rom_version;
    ind.btss_patch_version = ho_inst->btss_patch_version;

    return (handoverProfile_SendHandoverMsg(&ind,
            sizeof(HANDOVER_PROFILE_VERSION_IND_T),
            HANDOVER_PROFILE_PROTOCOL_MSG_TIMEOUT_MSEC));
}
...
handover_profile_status_t HandoverProfile_ProcessProtocolStartCfm(void)
{
    ...
    if((buf = HandoverProfile_BlockReadSource(ho_inst->link_source, HANDOVER_PROTOCOL_START_CFM, &size, HANDOVER_PROFILE_PROTOCOL_MSG_TIMEOUT_MSEC)) == NULL)
    {
        DEBUG_LOG_INFO("HandoverProfile_ProcessProtocolStartCfm timedout waiting for HANDOVER_START_CFM.");
        return HANDOVER_PROFILE_STATUS_HANDOVER_TIMEOUT;
    }
    ...
    {
        DEBUG_LOG_INFO("HandoverProfile_ProcessProtocolStartCfm status was not success. Session id=%u, cfm=0x%x", buf[1], buf[2]);
...
handover_profile_status_t HandoverProfile_ProcessProtocolUnmarshalP1Cfm(void)
{
    ...
    /* Block on peer stream to receive HANDOVER_START_CFM */
    if((HandoverProfile_BlockReadSource(ho_inst->link_source, HANDOVER_PROTOCOL_UNMARSHAL_P1_CFM, &size, HANDOVER_PROFILE_PROTOCOL_MSG_TIMEOUT_MSEC)) == NULL)
    {
        DEBUG_LOG_INFO("HandoverProfile_ProcessProtocolUnmarshalP1Cfm timedout waiting for HANDOVER_PROTOCOL_UNMARSHAL_P1_CFM.");
        return HANDOVER_PROFILE_STATUS_HANDOVER_TIMEOUT;
    }
...
/*! 
    \brief Provides message size for a given message id.
    \param[in] id Handover Protocol message id
    \return size of message if message id is valid, 0 otherwise
*/    
static inline uint8 handoverProfile_GetMessageSize(uint8 id)
{
    switch(id)
    {
        case HANDOVER_PROTOCOL_START_CFM:
            return sizeof(HANDOVER_PROFILE_HANDOVER_START_CFM_T);
        case HANDOVER_PROTOCOL_UNMARSHAL_P1_CFM:
            return sizeof(HANDOVER_PROFILE_HANDOVER_UNMARSHAL_P1_CFM_T);
        default:
            return 0;
    }
}
    ...
    uint32 timeout)
{
    *size = 0;
    ...
    do
    {
        ...
        {
            /* Got some data */
            buffer = (uint8*)SourceMap(src);

            /* Check for requested message id */
            if(messageid == HANDOVER_PROTOCOL_ANY_MSG_ID ||                    
               (messageid == handoverProfile_GetMessageId(buffer) &&
                *size == handoverProfile_GetMessageSize(messageid)))
            {
                return buffer;
            }
            else
            {
                /* Drop unexpected message and wait again */
                DEBUG_LOG_INFO("HandoverProfile_BlockReadSource: Dropping unexpected message ID: %d, Size: %d", 
                          handoverProfile_GetMessageId(buffer), *size);
                SourceDrop(src, *size);
                buffer = NULL;
            }
        }
...
void HandoverProfile_ProcessHandoverMessage(const MessageMoreData *mmd)
{
    ...
    while(size != 0)
    {
        ...
        switch (opcode)
        {
            case HANDOVER_PROFILE_VERSION_IND:
                handoverProfile_ProcessVersionInd(data, size);
                block = FALSE;
                break;
        ...
        if(unmarshal_complete)
        {
            ...
            {
                DEBUG_LOG_INFO("handoverProfile_ProcessHandoverMessage timeout waiting to re-enter sniff mode");
            }
            ...
            DEBUG_LOG_INFO("handoverProfile_ProcessHandoverMessage Handover Complete. I am new Primary.");
            return;
        }
        size = 0;

        /* Wait for more data in case of HANDOVER_PROTOCOL_START_REQ and HANDOVER_MARSHAL_DATA */
        if(HandoverProfile_BlockReadSource(ho_inst->link_source, HANDOVER_PROTOCOL_ANY_MSG_ID, &size, HANDOVER_PROFILE_PROTOCOL_MSG_TIMEOUT_MSEC) == NULL)
        {
            if(opcode == HANDOVER_MARSHAL_DATA)
            {
                ...
                {
                    DEBUG_LOG_INFO("handoverProfile_ProcessHandoverMessage timedout waiting for P1 marshal data. Cleanup P1 clients");
                    ho_inst->marshal_state = HANDOVER_PROFILE_MARSHAL_STATE_IDLE;
                    /* Call abort to free up any P1 unmarshalled data */
                    HandoverProfile_AbortP1Clients();
                }
...
handover_profile_status_t HandoverProfile_MarshalP0Clients(const tp_bdaddr *bd_addr)
{
    ...
    if(marshal_src == 0)
    {
        DEBUG_LOG_INFO("HandoverProfile_MarshalP0Clients StreamAclMarshalSource failed!!");
        return HANDOVER_PROFILE_STATUS_HANDOVER_TIMEOUT;
    }
    ...
    while(p0_data_available)
    {
        ...
        if((p0_data_len=SourceSize(marshal_src)) != 0)
        {
            ...
            if(write_ptr == NULL)
            {
                ...
                if(write_ptr ==  NULL)
                {
                    DEBUG_LOG_INFO("HandoverProfile_MarshalP0Clients handoverProfile_ClaimSink failed!!");
                    return HANDOVER_PROFILE_STATUS_HANDOVER_TIMEOUT;
                }
        ...
        if(packet_full && p0_data_available)
        {
            ...
            {
                DEBUG_LOG_INFO("HandoverProfile_MarshalP0Clients Due to SinkFlush failure, unable to send packet to peer ");
    ...
    else
    {
        ...
        if(write_ptr == NULL)
        {
            DEBUG_LOG_INFO("HandoverProfile_MarshalP0Clients handoverProfile_ClaimSink failed/timedout!!");
            return HANDOVER_PROFILE_STATUS_HANDOVER_TIMEOUT;
        }
    ...
    if(SinkFlush(ho_inst->link_sink, ho_inst->sink_written) == 0)
    {
        DEBUG_LOG_INFO("HandoverProfile_MarshalP0Clients Unable to flush end of data tag");
        return HANDOVER_PROFILE_STATUS_HANDOVER_FAILURE;
    }
...
handover_profile_status_t HandoverProfile_MarshalP1Clients(const tp_bdaddr *bd_addr)
{
    ...
    while(client_id < ho_inst->num_clients)
    {
        ...
        if(write_ptr == NULL)
        {
            ...
            if(dest_addr == NULL)
            {
                DEBUG_LOG_INFO("HandoverProfile_MarshalP1Clients handoverProfile_ClaimSink failed/timedout!!");
                return HANDOVER_PROFILE_STATUS_HANDOVER_TIMEOUT;
            }
        ...
        else
        {
            ...
            if(SinkFlush(ho_inst->link_sink, ho_inst->sink_written) == 0)
            {
                DEBUG_LOG_INFO("HandoverProfile_MarshalP1Clients Unable to flush the packet");
                return HANDOVER_PROFILE_STATUS_HANDOVER_FAILURE;
            }
    ...
    if(ho_inst->sink_written)
    {
        ...
        if(SinkFlush(ho_inst->link_sink, ho_inst->sink_written) == 0)
        {
            DEBUG_LOG_INFO("HandoverProfile_MarshalP1Clients Unable to flush the last P1 packet");
            return HANDOVER_PROFILE_STATUS_HANDOVER_FAILURE;
        }
...
void HandoverProfile_HandlePrepareForMarshalFailure(const tp_bdaddr *addr)
{
    ...
    {
        DEBUG_LOG_INFO("HandoverProfile_HandlePrepareForMarshalFailure Failed to send Handover Cancel Ind");
    }
    ...
    {
        DEBUG_LOG_INFO("HandoverProfile_HandlePrepareForMarshalFailure AclReceiveEnable timedout/failed");
    }
...
void HandoverProfile_HandleMarshalFailure(const tp_bdaddr *addr)
{
    ...
    if(HandoverProfile_SendHandoverCancelInd() != HANDOVER_PROFILE_STATUS_SUCCESS)
    {
        DEBUG_LOG_INFO("HandoverProfile_HandleMarshalFailure Failed to send Handover Cancel Ind");
    }

    if(!AclHandoverCancel(ho_inst->acl_handle))
    {
        DEBUG_LOG_INFO("HandoverProfile_HandleMarshalFailure AclHandoverCancel failed");
    }
    ...
    if(!AclReceiveEnable(addr, TRUE, HANDOVER_PROFILE_ACL_RECEIVE_ENABLE_TIMEOUT_USEC))
    {
        DEBUG_LOG_INFO("HandoverProfile_HandleMarshalFailure AclReceiveEnable timedout/failed");
    }
...
/*! \brief Wait for a packet to be processed by the transform connecting the
    ...
    \param time_to_wait The maximum number of milli-seconds to wait for a packet
           to be processed.
 */
static void handoverProfile_WaitForPacket(uint32 time_to_wait)
{
    Transform trans;
    ...
    if (trans && time_to_wait)
    {
        uint32 timeout = VmGetClock() + time_to_wait;
...
handover_profile_status_t HandoverProfile_PrepareForMarshal(const tp_bdaddr *addr)
{
    ...
    /* If A2DP is streaming, the time between received packets can be
    reduced by starting handover immediately after a packet is received from the
    handset. A proportion of the handover time will then occur in the gap before
    the next packet. This increases the overall handover time from the
    perspective of the procedure that initiates the handover (since the software
    waits for a packet before even starting to handover), but reduces the chance
    of there being a audio glitch as the packet that is received can be decoded
    and rendered whilst the handover is performed. */
    handoverProfile_WaitForPacket(HANDOVER_PROFILE_A2DP_HANDOVER_WAIT_FOR_PACKET_TIMEOUT_MS);

    HandoverPioSet();
    if(!AclReceiveEnable(addr, FALSE, HANDOVER_PROFILE_ACL_RECEIVE_ENABLE_TIMEOUT_USEC))
    {
        DEBUG_LOG_INFO("HandoverProfile_PrepareForMarshal AclReceiveEnable(false) timeout/failed");
        HandoverPioClr();
        return HANDOVER_PROFILE_STATUS_HANDOVER_TIMEOUT;
    }
    HandoverPioClr();
    ...
    {
        DEBUG_LOG_INFO("HandoverProfile_PrepareForMarshal AclReceivedDataProcessed timeout/failed, ret=0x%x", ret);
        return HANDOVER_PROFILE_STATUS_HANDOVER_TIMEOUT;
    }

    if(HandoverProfile_IsAclTransmitPending(addr, HANDOVER_PROFILE_ACL_TRANSMIT_DATA_PENDING_TIMEOUT_MSEC))
    {
        DEBUG_LOG_INFO("HandoverProfile_PrepareForMarshal AclTransmitDataPending timeout/failed");
        return HANDOVER_PROFILE_STATUS_HANDOVER_TIMEOUT;
    }
    ...
    {
        HandoverPioClr();
        DEBUG_LOG_INFO("HandoverProfile_PrepareForMarshal Vetoed by P1");
        return HANDOVER_PROFILE_STATUS_HANDOVER_VETOED;
    }
    HandoverPioClr();
    ...
    */
    ...
    {
        HandoverPioClr();
        DEBUG_LOG_INFO("HandoverProfile_PrepareForMarshal No connection exists with the Handset");
        return HANDOVER_PROFILE_STATUS_HANDOVER_TIMEOUT;
    }
    ...
    {
        DEBUG_LOG_INFO("HandoverProfile_PrepareForMarshal AclHandoverPrepare failed, ho_inst->acl_handle=0x%x", ho_inst->acl_handle);
        return HANDOVER_PROFILE_STATUS_HANDOVER_VETOED;
    }

    if(timedout)
    {
        DEBUG_LOG_INFO("HandoverProfile_PrepareForMarshal Handover timedout");
        return HANDOVER_PROFILE_STATUS_HANDOVER_TIMEOUT;
    }
