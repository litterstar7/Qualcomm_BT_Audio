...
handover_profile_status_t HandoverProfile_Handover(const tp_bdaddr *remote_addr)
{
    ...
    switch (ho_inst->secondary_firmware)
    {
        case HANDOVER_PROFILE_SECONDARY_FIRMWARE_UNKNOWN:
            DEBUG_LOG_INFO("HandoverProfile_Handover secondary firmware unknown - veto");
            return HANDOVER_PROFILE_STATUS_HANDOVER_VETOED;

        case HANDOVER_PROFILE_SECONDARY_FIRMWARE_MISMATCHED:
            DEBUG_LOG_INFO("HandoverProfile_Handover secondary firmware mismatched");
    ...
    if(ho_inst->is_primary)
    {
        ...
        {
            DEBUG_LOG_INFO("HandoverProfile_Handover P1 Client veto'ed before start request sent to peer");
            return HANDOVER_PROFILE_STATUS_HANDOVER_VETOED;
        }
        ...
        {
            DEBUG_LOG_INFO("HandoverProfile_Handover failed to send HANDOVER_START_REQ to Peer");
            return ret;
        }
        ...
        {
            DEBUG_LOG_INFO("HandoverProfile_Handover HandoverProfile_ProcessProtocolStartCfm wasn't success. ret:%d", ret);
            return ret;
        }
        ...
        if(ret != HANDOVER_PROFILE_STATUS_SUCCESS)
        {
            ...
            {
                DEBUG_LOG_INFO("HandoverProfile_Handover Failed to send Handover Cancel Ind");
            }
            HandoverProfile_AbortP1Clients();
            DEBUG_LOG_INFO("HandoverProfile_Handover HandoverProfile_Marshal_P1_Clients timeout/failed");
            ho_inst->marshal_state = HANDOVER_PROFILE_MARSHAL_STATE_IDLE;
            return ret;
        }
        ...
        if((ret = HandoverProfile_ProcessProtocolUnmarshalP1Cfm())  != HANDOVER_PROFILE_STATUS_SUCCESS)
        {
            ...
            DEBUG_LOG_INFO("HandoverProfile_Handover Failed to receive Unmarshal P1 confimration message");
            ho_inst->marshal_state = HANDOVER_PROFILE_MARSHAL_STATE_IDLE;
            HandoverProfile_AbortP1Clients();
            return ret;
        }
        ...
        if(HandoverProfile_VetoP1Clients())
        {
            ...
            DEBUG_LOG_INFO("HandoverProfile_Handover P1 Client veto'ed before AclPrepare");
            ho_inst->marshal_state = HANDOVER_PROFILE_MARSHAL_STATE_IDLE;
            HandoverProfile_AbortP1Clients();
            return HANDOVER_PROFILE_STATUS_HANDOVER_VETOED;
        }
        ...
        if((ret=HandoverProfile_PrepareForMarshal(&ho_inst->tp_handset_addr)) != HANDOVER_PROFILE_STATUS_SUCCESS)
        {
            ...
            */
            ...
            DEBUG_LOG_INFO("HandoverProfile_Handover Prepare for Marshal failed. ret:%d", ret);
            return ret;
        }
        ...
        if(HandoverProfile_VetoP1Clients())
        {
            ...
            DEBUG_LOG_INFO("HandoverProfile_Handover Vetoed by P1 after AclHandoverPrepare succeeded");
            HandoverPioClr();
            return HANDOVER_PROFILE_STATUS_HANDOVER_VETOED;
        }
        HandoverPioClr();
        ...
        if(ret != HANDOVER_PROFILE_STATUS_SUCCESS)
        {
            ...
            */
            HandoverProfile_HandleMarshalFailure(&ho_inst->tp_handset_addr);
            DEBUG_LOG_INFO("HandoverProfile_Handover HandoverProfile_Marshal_P0_Clients timeout/failed");
            return ret;
        }
        ...
        */
        ...
        {
            DEBUG_LOG_INFO("HandoverProfile_IsAclTransmitPending timeout");
        }
        ...
        {
            DEBUG_LOG_INFO("HandoverProfile_Handover AclHandoverCommit() failed at Primary, acl handle=0x%x. Panic!!", ho_inst->acl_handle);
            /* Send HANDOVER_PROTOCOL_CANCEL_IND to secondary device and Panic */
            if(HandoverProfile_SendHandoverCancelInd() != HANDOVER_PROFILE_STATUS_SUCCESS)
            {
                DEBUG_LOG_INFO("HandoverProfile_Handover handoverProfile_SendHandoverCancelInd failed. Panic!!");
            }
            Panic();
        }
        ...
        {
            DEBUG_LOG_INFO("HandoverProfile_Handover timeout waiting to re-enter sniff mode");
        }
        ...
        DEBUG_LOG_INFO("HandoverProfile_Handover Handover Complete. I am new Secondary.");

        return HANDOVER_PROFILE_STATUS_SUCCESS;
    }
    DEBUG_LOG_INFO("HandoverProfile_Handover Failed as instance was not primary");
    return HANDOVER_PROFILE_STATUS_HANDOVER_FAILURE;
}
