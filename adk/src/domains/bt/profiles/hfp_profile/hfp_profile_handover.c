...
const marshal_type_t hfp_profile_marshal_types[] = {
    MARSHAL_TYPE(hfpInstanceTaskData),
};
...
static bool hfpProfile_Veto(void)
{
    ...
    {
        DEBUG_LOG_INFO("hfpProfile_Veto, Messages pending for HFP task");
        veto = TRUE;
    }
    else 
    {
        if (appHfpGetLock())
        {
            DEBUG_LOG_INFO("hfpProfile_Veto, hfp_lock");
            veto = TRUE;
        }
        else if (!appHfpIsDisconnected() && appGetHfp()->bitfields.detach_pending)
        {
             ...
             */
            DEBUG_LOG_INFO("hfpProfile_Veto, detach_pending");
            veto = TRUE;
        }
    }
