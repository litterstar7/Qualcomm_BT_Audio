...
static void hfpProfile_MessageHandler(Task task, MessageId id, Message message);

/* Make the type used for message IDs available in debug tools */
LOGGING_PRESERVE_MESSAGE_ENUM(hfp_profile_messages)
LOGGING_PRESERVE_MESSAGE_ENUM(hfp_profile_internal_messages)
TaskData hfp_profile_task_structure = { .handler = hfpProfile_MessageHandler};
...
/*! \brief HFP instance task data. */
hfpInstanceTaskData appHfp;
...
/*! \brief volume change 1 step only
*/
static bool appHfpVolumeStep(int16 step)
{
    DEBUG_LOG("appHfpVolumeStep(%d)", step);
...
static void appHfpHandleUiInput(MessageId  ui_input)
{
    switch (ui_input)
    {
        ...
        case ui_input_hfp_volume_up:
            appHfpVolumeStep(hfpConfigGetHfpVolumeStep());
            break;

        case ui_input_hfp_volume_down:
            appHfpVolumeStep(-hfpConfigGetHfpVolumeStep());
            break;
...
#ifdef INCLUDE_AV_SUSPEND_REASON_HFP
    /* Resume AV streaming */
...
static void appHfpSetState(hfpState state)
{
    ...
    Ui_InformContextChange(ui_provider_telephony, VoiceSources_GetSourceContext(voice_source_hfp_1));
    ...
    switch (appHfpGetState())
    {
        ...
        case HFP_STATE_CONNECTING_REMOTE:
        {
            ...
            if (cfm->status == hfp_audio_connect_success)
            {
                ...
                /* Update the SCO context */
                if(HFP_STATE_CONNECTED_ACTIVE == appHfpGetState())
                {
                    Ui_InformContextChange(ui_provider_telephony, VoiceSources_GetSourceContext(voice_source_hfp_1));
                }
...
#if defined(INCLUDE_AV) && defined(SUSPEND_ON_VOICE_RECOGNITION) && defined(INCLUDE_AV_SUSPEND_REASON_HFP)
...
static void appHfpHandleHfpHfIndicatorsInd(const HFP_HF_INDICATORS_IND_T *ind)
{
    ...
    if (ind->hf_indicator_assigned_num == hf_battery_level)
    {
        ...
        {
            appGetHfp()->bitfields.hf_indicator_assigned_num = hf_battery_level;
            appBatteryRegister(&hfp_profile_task_data.battery_form);
            HfpBievIndStatusRequest(hfp_primary_link, hf_battery_level, appBatteryGetPercent());
        }
        else
        {
            appBatteryUnregister(hfp_profile_task_data.battery_form.task);
        }
    }
}
...
static void hfpProfile_InitVolume(voice_source_t source, uint8 init_volume)
{
    volume_t volume;
...
static void hfpProfile_InitTaskData(void)
{
    ...
    hfp_profile_task_data.battery_form.task = &hfp_profile_task_structure;
    hfp_profile_task_data.battery_form.hysteresis = 1;
    hfp_profile_task_data.battery_form.representation = battery_level_repres_percent;
}

static void hfpProfile_InitInstanceTaskData(unsigned mic_volume)
{
    /* Set up task handler */
    appGetHfp()->task.handler = appHfpHandleMessage;

    /* Update configuration */
    appGetHfp()->mic_volume = mic_volume;
...
bool appHfpInit(Task init_task)
{
    ...
    VoiceSources_RegisterObserver(voice_source_hfp_1, HfpProfile_GetVoiceSourceObserverInterface());

    hfpProfile_InitVolume(voice_source_hfp_1, ps_config.volume);

    hfpProfile_InitInstanceTaskData(ps_config.mic_volume);
...
{
    hfp_profile_task_data.at_cmd_task = task;
}
...
static void hfpProfile_MessageHandler(Task task, MessageId id, Message message)
{
    ...
    DEBUG_LOG("hfpProfile_MessageHandler id 0x%x", id);
