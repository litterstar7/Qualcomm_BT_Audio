...
static void a2dpProfile_SetState(audio_source_t source, audio_source_state_t state);
...
static bool a2dpProfile_GetConnectParameters(audio_source_t source, a2dpTaskData * a2dp, source_defined_params_t * source_params)
{
    ...
    PanicNull(a2dp);
    DEBUG_LOG("a2dpProfile_GetConnectParameters source=%d a2dp=%p, device_id=%d, stream_id=%d", source,
    ...
    if(codec_settings)
    {
        ...
        audio_connect_params->volume = Volume_CalculateOutputVolume(AudioSources_GetVolume(source));
...
static bool a2dpProfile_IsAudioAvailable(audio_source_t source)
{
    ...
    if((source == audio_source_a2dp_1 || source == audio_source_a2dp_2) && appAvGetA2dpSink(AV_CODEC_ANY))
    {
        is_available = TRUE;
    }
    return is_available;
...
static void a2dpProfile_SetState(audio_source_t source, audio_source_state_t state)
{
    ...
    if(av_instance)
    {
        A2dpProfile_SetSourceState(av_instance, state);
    }
}
