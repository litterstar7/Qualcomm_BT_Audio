...
static void MirrorProfile_HandleAudioSyncConnectInd(const AUDIO_SYNC_CONNECT_IND_T *ind)
{
    if(MirrorProfile_StoreAudioSourceParameters(ind->source_id))
    {
        ...
        MessageSendConditionally(ind->task, AUDIO_SYNC_CONNECT_RES, rsp, MirrorProfile_GetStreamChangeLockAddr());
...
static void mirrorProfile_MessageHandler(Task task, MessageId id, Message message)
{
    ...
    case MIRROR_INTERNAL_START_AUDIO_SYNC:
...
/*! \brief A2DP profile calls this function when it wants to start the audio
           chains. If mirroring connected and enabled then starting the audio
           chains is deferred until the A2DP mirror is connected. Otherwise
           call back to the original A2DP function to immediately start
    \param source The audio source.
*/
static void mirrorProfile_SendAudioConnectMessage(audio_source_t source)
{
    DEBUG_LOG("mirrorProfile_SendAudioConnectMessage");
    if (MirrorProfile_IsConnected() && MirrorProfile_IsA2dpMirroringEnabled())
    {
        MirrorProfile_Get()->a2dp_source_to_connect = source;
    }
    else
    {
        MirrorProfile_Get()->SendAudioConnect(source);
        MirrorProfile_Get()->a2dp_source_to_connect = audio_source_none;
    }
}

/*! \brief A2DP profile calls this function when it wants to stop the audio
           chains. Mirror profile just calls back into the original A2DP
           function to stop the audio chains immediately.
    \param source The audio source.
*/
static void mirrorProfile_SendAudioDisconnectMessage(audio_source_t source)
{
    ...
    MirrorProfile_Get()->SendAudioDisconnect(source);
}
...
bool MirrorProfile_Init(Task task)
{
    ...
    /* Override A2DP profile's messaging to control audio start/stop.
       Mirror profile will call back to send the messages when it is prepared
       to start the audio chains */
    mirror_profile.SendAudioConnect = A2dpProfile_SetSendAudioConnectMessageFunction(mirrorProfile_SendAudioConnectMessage);
    mirror_profile.SendAudioDisconnect = A2dpProfile_SetSendAudioDisconnectMessageFunction(mirrorProfile_SendAudioDisconnectMessage);
...
void MirrorProfile_HandleHfpAudioConnectConfirmation(voice_source_t source)
{
        ...
        && MirrorProfile_IsVoiceSourceSupported(voice_source_hfp_1))
    {
        /* Defer starting eSCO audio until eSCO mirroring is connected.
           ...
           start. */
        MirrorProfile_Get()->voice_source_to_connect = source;
    }
    else
    {
