                                                            ...
                                                            Message goal_data)
{
    ...
    HandsetService_DisconnectRequest(HeadsetTopProcDisconnectHandsetGetTask(), &handset_addr, 0);
...
static void headsetTopology_ProcDisconnectHandsetHandleHandsetDisconnectCfm(const HANDSET_SERVICE_DISCONNECT_CFM_T *cfm)
{
    ...
    DEBUG_LOG_VERBOSE("headsetTopology_ProcDisconnectHandsetHandleHandsetDisconnectCfm status %d", cfm->status);

    td->complete_fn(hs_topology_procedure_disconnect_handset, procedure_result_success);
    headsetTopology_ProcDisconnectHandsetResetProc();
    HeadsetTopology_SendHandsetDisconnectedIndication(cfm);
}
...
static void headsetTopology_ProcDisconnectHandsetHandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        default:
            DEBUG_LOG_VERBOSE("headsetTopology_ProcDisconnectHandsetHandleMessage unhandled id 0x%x(%d)", id, id);
            break;
    }
}
