...
{
    headsetTopProcConnectHandsetTaskData* td = HeadsetTopProcConnectHandsetGetTaskData();
    DEBUG_LOG_VERBOSE("headsetTopology_ProcConnectHandsetHandleHandsetConnectCfm status %d", cfm->status);
    ...
    if (cfm->status != handset_service_status_cancelled)
    {
        ...
        {
            device_t device = BtDevice_GetDeviceForBdAddr(&cfm->addr);
            uint32 profiles_connected = BtDevice_GetConnectedProfiles(device);
            DEBUG_LOG("headsetTopology_ProcConnectHandsetHandleHandsetConnectCfm, connected_profiles:0x%x", profiles_connected);
...
{
    DEBUG_LOG_VERBOSE("headsetTopology_ProcConnectHandsetHandleConMgrConnInd Transport:%d Type:%d, Tpaddr:x%06x Incoming connection:%d",
...
static void headsetTopology_ProcConnectHandsetHandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case CON_MANAGER_TP_DISCONNECT_IND:
            DEBUG_LOG_VERBOSE("headsetTopology_ProcConnectHandsetHandleMessage: CON_MANAGER_TP_DISCONNECT_IND");
            break;

        default:
            DEBUG_LOG_VERBOSE("headsetTopology_ProcConnectHandsetHandleMessage unhandled id 0x%x(%d)", id, id);
            break;
    }
}
