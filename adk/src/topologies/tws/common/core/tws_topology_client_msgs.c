...
void TwsTopology_SendStopCfm(tws_topology_status_t sts)
{
    ...
    DEBUG_LOG("twsTopology_SendStopCfm sts %u", sts);

    MessageCancelAll(TwsTopologyGetTask(), TWSTOP_INTERNAL_TIMEOUT_TOPOLOGY_STOP);

    message->status = sts;
    MessageSend(twst->app_task, TWS_TOPOLOGY_STOP_CFM, message);
}
