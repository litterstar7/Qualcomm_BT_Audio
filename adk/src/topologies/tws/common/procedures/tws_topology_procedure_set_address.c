...
const SET_ADDRESS_TYPE_T proc_set_address_primary = { .primary = TRUE };
const SET_ADDRESS_TYPE_T proc_set_address_primary_no_panic = { .primary = TRUE, .do_not_panic = TRUE };
const SET_ADDRESS_TYPE_T proc_set_address_secondary = { . primary = FALSE };
                                          ...
                                          Message goal_data)
{
    ...
    if (type->primary)
    {
        bool found = appDeviceGetPrimaryBdAddr(&td->addr);

        if (!found)
        {
            if (type->do_not_panic)
            {
                DEBUG_LOG_WARN("TwsTopology_ProcedureSetAddressStart, NO PRIMARY ADDRESS - NOT FOUND");
                change_address = FALSE;
            }
            else
            {
                DEBUG_LOG_ERROR("TwsTopology_ProcedureSetAddressStart, COULD NOT SWITCH TO PRIMARY ADDRESS - NOT FOUND");
                Panic();
            }
        }
        else
