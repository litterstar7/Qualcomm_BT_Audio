...
static void twsTopology_ProcHandoverStart(void)
{
    ...
    if(appDeviceIsHandsetConnected() && appDeviceGetHandsetBdAddr(&address) && TwsTopologyGetTaskData()->handover_info.reason)
    {
...
static void twsTopology_ProcHandoverHandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        {
            DEBUG_LOG("Cancel handover retry and complete the handover proc");
            twsTopology_ProcHandoverReset();
            Procedures_DelayedCompleteCfmCallback(TwsTopProcHandoverGetTaskData()->complete_fn, tws_topology_procedure_handover, procedure_result_failed);
        }
        break;
        default:
        break;
