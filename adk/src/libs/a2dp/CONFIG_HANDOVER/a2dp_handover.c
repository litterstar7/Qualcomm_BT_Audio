...
static void stitchRemoteDevice(remote_device *unmarshalled_remote_conn, const bdaddr *bd_addr)
{
    ...
    sink_ptr = &unmarshalled_remote_conn->signal_conn.connection.active.sink;
...
static void a2dpHandoverCommit(const tp_bdaddr *tp_bd_addr, const bool newRole)
{
    ...
    if (newRole && a2dp_marshal_inst)
    {
        ...
        /* Stitch connection information to A2DP instance */
        stitchRemoteDevice(a2dp_marshal_inst->data->remote_conn, &tp_bd_addr->taddr.addr);
    }
}
...
static void a2dpHandoverComplete( const bool newRole )
{
    if (newRole && a2dp_marshal_inst)
    {
        ...
        remote_device *unmarshalled_remote_conn, *remote_conn;

        PanicNull(a2dp_marshal_inst->data);

        unmarshalled_remote_conn = a2dp_marshal_inst->data->remote_conn;
        remote_conn = a2dpAddDevice(&unmarshalled_remote_conn->bd_addr);

        PanicNull(remote_conn);

        device_id = remote_conn->bitfields.device_id;

        unmarshalled_remote_conn->bitfields.device_id = device_id;

        stitchDataBlockHeader(a2dp_marshal_inst->data->data_blocks, device_id);
