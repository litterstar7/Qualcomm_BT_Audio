...
static void handleGattConnectCfm(const GATT_CONNECT_CFM_T * cfm)
{
    if (gatt_status_initialising == cfm->status)
    {
        ...
        {
            gattManagerConnectAsCentralInitialising(cfm->cid);
            GATT_MANAGER_DEBUG_INFO(("GM: Remote Server connection Initialising!"));
        }
    }
    else if (cfm->cid == gattManagerDataGetConnectAsPeripheralRemoteCid())
    {
        gattManagerConnectAsPeripheralConnected(cfm);
    }
    else
    {
        gattManagerConnectAsCentralCfm(cfm);
    }
}
...
static void gattMessageHandler(MessageId id, Message payload)
{
    ...
    switch(id)
    {
        ...
        case GATT_CONNECT_IND:
            gattManagerConnectInd((const GATT_CONNECT_IND_T*)payload);
            break;
        ...
        default:
            GATT_MANAGER_DEBUG_PANIC(("GM: Un-handled GATT message MESSAGE:0x%x\n", id));
            break;
    }
}
...
static void gattManagerInternalMessageHandler(MessageId id, Message payload)
{
    switch(id)
    {
        case GATT_MANAGER_INTERNAL_MSG_CANCEL_ADVERTISING:
            GattManagerCancelConnectAsPeripheral();
            break;

        case GATT_MANAGER_INTERNAL_MSG_CONNECT_AS_PERIPHERAL:
            gattManagerConnectAsPeripheralInternal((const GATT_MANAGER_INTERNAL_MSG_CONNECT_AS_PERIPHERAL_T *)payload);
            break;

        case GATT_MANAGER_INTERNAL_MSG_CONNECT_AS_CENTRAL:
            GattManagerConnectAsCentralInternal((const GATT_MANAGER_INTERNAL_MSG_CONNECT_AS_CENTRAL_T *)payload);
            break;

        default:
            GATT_MANAGER_DEBUG_PANIC(("ERROR: Un-handled Internal GATT Manager message MESSAGE:gatt_manager_internal_msg_t:0x%x\n", id));
            break;
    }
}
...
void gattManagerMessageHandler(Task task, MessageId id, Message payload)
{
    ...
    {
        GATT_MANAGER_DEBUG_PANIC(("ERROR: Un-handled message MESSAGE:0x%x\n", id));
    }
}
