...
{        
    if (ind->psm == AVCTP_PSM)
    {
        ...
        avrcp = avrcpFindTask(&ind->bd_addr, TRUE, TRUE);
        if (avrcp)
        {
            if (avrcp->bitfields.state != avrcpReady)
            {
                ...
                avrcp = avrcpCreateTask(NULL, avrcpConnecting, &ind->bd_addr, TRUE);
        ...
        else
        {
            ...
            avrcp = avrcpCreateTask(avrcpGetDeviceTask()->app_task, avrcpConnecting, &ind->bd_addr, TRUE);
        }
