...
static void avrcpHandoverCommit(const tp_bdaddr *tp_bd_addr, const bool newRole)
{
    ...
    if (newRole && avrcp_marshal_inst)
    {
        ...
        stitchAvrcp(avrcp_marshal_inst->avrcp);
    }
}
...
static void avrcpHandoverComplete( const bool newRole )
{
    if (newRole && avrcp_marshal_inst)
    {
        ...
        avrcpAddTaskToList(avrcp_marshal_inst->avrcp, &avrcp_marshal_inst->bd_addr, avrcp_marshal_inst->avrcp->bitfields.connection_incoming);

        /* Set the handover policy */
        Source src = StreamSourceFromSink(avrcp_marshal_inst->avrcp->sink);
...
static bool avrcpVeto( void )
{
    ...
    /* check if any avrcp instance is in CONNECTING stage*/
    while (list != NULL)
    {
