...
LOGGING_PRESERVE_MESSAGE_TYPE(BroadcastMessageId)
LOGGING_PRESERVE_MESSAGE_TYPE(BROADCAST_INTERNAL_MSG)
...
static void broadcast_lib_core_msg_handler(Task task, MessageId id, Message message)
{
    ...
    BROADCAST_DEBUG(("BL:CORE: Message RX with ID MESSAGE:%x\n", id));
}
...
static void broadcast_msg_handler(Task task, MessageId id, Message message)
{
    ...
    {
        BROADCAST_DEBUG(("BL: Unknown message received MESSAGE:0x%x\n", id));
    }
}
