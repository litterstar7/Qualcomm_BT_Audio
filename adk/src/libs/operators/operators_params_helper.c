...
{
    unsigned memory_size = (unsigned) sizeof(get_status_data_t) + number_of_params * (unsigned) sizeof(standard_param_value_t);
    get_status_data_t* get_status_data = PanicUnlessMalloc(memory_size);
    memset(get_status_data, 0, memory_size);
    get_status_data->number_of_params = number_of_params;
