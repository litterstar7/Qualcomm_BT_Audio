        ...
        switch(path)
        {
            case AUDIO_ANC_PATH_ID_FFA:
            {
                ...
                if(is_fine_gain)
                    ...
                    *gain = (gain_config->gain_shift & 0xF);
            }
            break;
            case AUDIO_ANC_PATH_ID_FFB:
            {
    ...
    if(instance)
    {
        switch(path)
        {
            case AUDIO_ANC_PATH_ID_FFA:
            {
                ...
                memcpy(denominator, &coefficients->coefficients[0], sizeof(uint32)*7);
                memcpy(numerator, &coefficients->coefficients[7], sizeof(uint32)*8);
            }
            break;
            case AUDIO_ANC_PATH_ID_FFB:
            {
