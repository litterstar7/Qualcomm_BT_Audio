...
bool ancCommonStateHandleSetMode(anc_state_event_t event)
{
    ...
    ANC_ASSERT((event.id == anc_state_event_set_mode) || (event.id == anc_state_event_set_mode_filter_coefficients));
    if (!((event.id == anc_state_event_set_mode) || (event.id == anc_state_event_set_mode_filter_coefficients)))
        return FALSE;
