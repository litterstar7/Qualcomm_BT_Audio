...
static void handleGattManagerMsg(Task task, MessageId id, Message payload)
{
    ...
    switch (id)
    {
        ...
        default:
        {
            ...
            GATT_BATTERY_CLIENT_DEBUG_PANIC(("BASC: Client GattMgr Msg not handled MESSAGE:0x%x\n", id));
        }
        break;
    }
}
...
static void handleGattMsg(MessageId id)
{
    ...
    GATT_BATTERY_CLIENT_DEBUG_PANIC(("BASC: Client Gatt Msg not handled MESSAGE:0x%x\n", id));
}
...
static void handleInternalBatteryMsg(Task task, MessageId id, Message payload)
{
    ...
    switch (id)
    {
        ...
        {
            /* Unrecognised GATT Manager message */
            GATT_BATTERY_CLIENT_DEBUG_PANIC(("BASC: Client Internal Msg not handled MESSAGE:battery_internal_msg_t:0x%x\n", id));
        }
        break;
    }
}
