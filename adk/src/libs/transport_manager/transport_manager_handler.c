...
bool TransportMgrCreateTransLinkData(Task application_task, transport_mgr_link_cfg_t link_cfg, Sink snk)
{
    ...
    trans_data = transportMgrFindDataWithTransInfo(link_cfg.trans_info.non_gatt_trans.trans_link_id, transportMgrConvertAdaptationType(link_cfg.type));
    if (trans_data)
    {
        /* Node found, Update sink */
        PRINT(("TransportMgrCreateTransLinkData: Node found\n",snk, link_cfg.trans_info.non_gatt_trans.trans_link_id));
        trans_data->sink = snk;
