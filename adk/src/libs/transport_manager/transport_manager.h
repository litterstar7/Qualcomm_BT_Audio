...
*/
 ...
 /*!
    @brief To claim space for data in the respective transport
    ...
    @return 
    Pointer to claimed space when the claim is successful
    else NULL
*/
 ...
 */
transport_mgr_status_t TransportMgrDataSend(transport_mgr_type_t type, uint16 trans_link_id, uint16 data_length);
...
/******************* APIs used for Creating Link Data during Handover ***********************/
/*!
    @brief Associate Sink with Transport manager Task
...
*/
void TransportMgrConfigureSink(Sink snk);

/*!
    @brief Create RFCOMM link data on new Primary during a handover commit.
    If rfcomm channel provided in link_cfg parameter is already present in a node, 
    it is updated with the provided sink and function returns TRUE,
    else new node is created and function returns FALSE.

	If TRUE is returned, new RFCOMM channel should be registered (since current is used up)
    and current SDP record should be removed and new created.

    If FALSE is returned, current SDP record should not be changed.
 
    @param application_task: GAIA's application task
    @param link_cfg : link conifguration (includes RFCOMM server channel)
    @param snk: Sink
.
...
/*!
    @brief Deletes RFCOMM link data on old Primary during a handover commit.
 
    @param trans_type: transport type (currently only RFCOMM handover supported)
    @param transport_id : RFCOMM channel ID
.
