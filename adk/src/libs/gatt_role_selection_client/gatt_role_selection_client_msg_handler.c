...
static void handleGattManagerMsg(Task task, MessageId id, Message payload)
{
    ...
    switch (id)
    {
        ...
        default:
            GATT_ROLE_SELECTION_CLIENT_DEBUG("handleGattManagerMsg: Unhandled message MESSAGE:0x%x\n", id);
            break;
    }
}
...
static void handleGattMsg(MessageId id)
{
    ...
    GATT_ROLE_SELECTION_CLIENT_DEBUG("handleGattMsg: Unhandled MESSAGE:0x%x\n", id);
...
static void handleInternalRoleSelectionMsg(Task task, MessageId id, Message message)
{
    switch(id)
    {
        ...
        default:
            GATT_ROLE_SELECTION_CLIENT_DEBUG("handleInternalRoleSelectionMsg: Unhandled MESSAGE:0x%x\n", id);
            break;
    }
}
