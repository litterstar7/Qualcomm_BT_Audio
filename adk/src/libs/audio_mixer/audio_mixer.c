...
LOGGING_PRESERVE_MESSAGE_TYPE(audio_mixer_message_t)
...
{
    uint32 sample_rate = input_sample_rate;

    if (AudioConfigGetMaximumConcurrentInputs() > 1)
    {
        sample_rate = AudioConfigGetOutputSampleRate();
    }
    else if(connection_type == CONNECTION_TYPE_MUSIC)
    {
        sample_rate = AudioConfigGetWiredAudioOutputSampleRate();
    }
