...
const marshal_type_descriptor_t mtd_int16 = MAKE_MARSHAL_TYPE_DEFINITION_BASIC(int16);
const marshal_type_descriptor_t mtd_int = MAKE_MARSHAL_TYPE_DEFINITION_BASIC(int);
...
/*! \brief The l2cap sinks are marshalled as l2cap CIDs. This function converts
           the unmarshal_sink (i.e. CID) to a real sink, then configures the sink.
*/
void configureL2capSinkFromMarshalledSinkCid(Sink *unmarshalled_sink, Task task)
{
    uint16 cid;
    Sink s;

    memcpy(&cid, unmarshalled_sink, sizeof(uint16));

    s = StreamL2capSink(cid);

    /* Associate cid with task in bluestack */
    VmOverrideL2capConnContext(cid, (conn_context_t)task);

    /* Associate the sink with the task for messages */
    MessageStreamTaskFromSink(s, task);
