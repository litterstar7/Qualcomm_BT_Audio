...
static void handle_hid_client_gatt_manager_msg(Task task, MessageId id, Message msg)
{
    ...
    GATT_HID_CLIENT_DEBUG_INFO(("GHIDC:Func: handle_hid_client_gatt_manager_msg(): MESSAGE:0x%x\n", id));
...
{
    GHIDC_T *const hid_client = (GHIDC_T*)task;
    GATT_HID_CLIENT_DEBUG_INFO(("GHIDC:Func: handle_hid_client_internal_msg(): MESSAGE:0x%x\n", id));
