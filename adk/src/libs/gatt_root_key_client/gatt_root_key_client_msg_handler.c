...
static void handleGattManagerMsg(Task task, MessageId id, Message payload)
{
    ...
    GATT_ROOT_KEY_CLIENT_DEBUG("GRKC handleGattManagerMsg: MESSAGE:0x%x\n", id);
    ...
    switch (id)
    {
        ...
        default:
            GATT_ROOT_KEY_CLIENT_DEBUG("handleGattManagerMsg: Unhandled message MESSAGE:0x%x\n", id);
            break;
    }
}
...
static void handleInternalRootKeyMsg(Task task, MessageId id, Message payload)
{
    ...
    switch (id)
    {
        ...
        default:
            GATT_ROOT_KEY_CLIENT_DEBUG("handleInternalRootKeyMsg: Unhandled MESSAGE:0x%x\n", id);
            break;
    }
}
