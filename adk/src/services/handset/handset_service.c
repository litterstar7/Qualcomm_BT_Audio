...
static handset_service_state_machine_t *handsetService_FindOrCreateSm(const tp_bdaddr *tp_addr)
{
...
static bool HandsetService_AuthoriseConnection(const bdaddr* bd_addr, dm_protocol_id protocol_id, uint32 channel, bool incoming)
{
    ...
    HS_LOG("HandsetService_AuthoriseConnection [%04x,%02x,%06lx] protocol %u, channel %lu, incoming %u", bd_addr->nap, bd_addr->uap, bd_addr->lap, 
                                                                                                         protocol_id, channel, incoming);
...
void HandsetService_ConnectMruRequest(Task task, uint32 profiles)
{
    bdaddr hs_addr;
