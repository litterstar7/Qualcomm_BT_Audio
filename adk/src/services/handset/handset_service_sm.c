...
unsigned HandsetServiceSm_GetBredrAclConnectionCount(void)
{
    ...
    for(sm = handset_service.state_machine; sm <= &handset_service.state_machine[HANDSET_SERVICE_MAX_SM - 1]; sm++)
    {
...
static void handsetServiceSm_EnterConnectingBredrProfiles(handset_service_state_machine_t *sm)
{
    ...
    HS_LOG("handsetServiceSm_EnterConnectingBredrProfiles to connect 0x%08x", sm->profiles_requested);
...
{
    uint32 connected_profiles = BtDevice_GetConnectedProfiles(sm->handset_device);
...
{
    uint32 profiles_connected = BtDevice_GetConnectedProfiles(sm->handset_device);
