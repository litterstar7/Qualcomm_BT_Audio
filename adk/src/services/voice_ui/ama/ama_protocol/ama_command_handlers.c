...
void AmaCommandHandlers_GetLocales(ControlEnvelope *control_envelope_in)
{
    ...
    Locale locale[MAX_AMA_LOCALES] = {LOCALE__INIT, LOCALE__INIT, LOCALE__INIT, LOCALE__INIT, LOCALE__INIT, LOCALE__INIT,
                                    LOCALE__INIT, LOCALE__INIT, LOCALE__INIT, LOCALE__INIT, LOCALE__INIT, LOCALE__INIT};
    ...
    for(uint8 i=0; i<supported_locales.num_locales; i++)
    {
        ...
        {
            locales.current_locale = &locale[i];
        }
    }
...
void AmaCommandHandlers_SetLocale(ControlEnvelope *control_envelope_in)
{
    ...
    FILE_INDEX locale_index = FileFind(FILE_ROOT, locale, strlen(locale));
    if(locale_index == FILE_NONE)
    {
        DEBUG_LOG("AMA COMMAND__SET_LOCALE Model NOT found for Locale = %c%c%c%c%c:",
                  locale[0], locale[1], locale[2], locale[3], locale[4]);
        response.error_code = ERROR_CODE__NOT_FOUND;
    }
    else
    {
        DEBUG_LOG("AMA COMMAND__SET_LOCALE Model found for Locale = %c%c%c%c%c:",
                  locale[0], locale[1], locale[2], locale[3], locale[4]);
