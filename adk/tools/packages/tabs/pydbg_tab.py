...
def get_elf_path_for_std_project(project_file, devkit, wsroot):
    """
    ...
    project = Project("std_project", project_file, sdk=devkit, wsroot=wsroot)
    chip_type = project.default_configuration.properties["CHIP_TYPE"]
    config_name = project.default_configuration.name

    project_path = os.path.dirname(os.path.abspath(project_file))
    build_dir = "_".join(["depend", config_name, chip_type])
    ...
    if len(elf_files) == 0:
        if not os.path.isdir(elf_dir):
            raise RuntimeError("Expected to find build directory {}, but it doesn't exist".format(elf_dir))
        raise RuntimeError("No files with .elf extension in expected build directory {}".format(elf_dir))
    elif len(elf_files) > 1:
        elf_names = [os.path.basename(f) for f in elf_files]
        raise RuntimeError("Multiple ELFs found in build directory {}: {}".format(elf_dir, ", ".join(elf_names)))
