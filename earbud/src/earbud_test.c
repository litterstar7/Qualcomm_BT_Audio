...
bool appTestAvVolumeChange(int8 step)
{
    ...
    VolumeService_ChangeAudioSourceVolume(audio_source_a2dp_1, step);
    return TRUE;
}
...
void appTestAvVolumeSet(uint8 volume)
{
    ...
    Volume_SendAudioSourceVolumeUpdateRequest(audio_source_a2dp_1, event_origin_local, volume);
}
...
static void testTaskHandler(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case KYMERA_NOTIFICATION_TONE_STARTED:
            {
                ...
                DEBUG_LOG("testTaskHandler KYMERA_NOTIFICATION_TONE_STARTED received tone %p", msg->tone);
        ...
        case KYMERA_NOTIFICATION_PROMPT_STARTED:
            {
                KYMERA_NOTIFICATION_PROMPT_STARTED_T *msg = (KYMERA_NOTIFICATION_PROMPT_STARTED_T *)message;
                DEBUG_LOG_VERBOSE("testTaskHandler KYMERA_NOTIFICATION_PROMPT_STARTED received prompt id 0x%x", msg->id);

                FILE_INDEX power_off_id = FileFind(FILE_ROOT, "power_off.sbc", strlen("power_off.sbc"));

                if(msg->id == power_off_id)
                {
...
static void earbudTest_ReportProperty(device_t device, device_property_t property)
{
    if (Device_IsPropertySet(device,property))
    {
        ...
        case device_property_last_connected_profiles:
            Device_GetPropertyU32(device, property, &u32_value);
            DEBUG_LOG_VERBOSE("\tlast_connected_profiles %08x", u32_value);
            break;
...
static void earbudTest_SendGaiaCommandToAnc(uint8 pdu_id, uint8 payload_size, uint8 payload)
{
     ...
     /* TODO: We need transport parameter here, what do we use? */
...
uint8 appTestGetAdaptiveAncFeedForwardGain(void)
{
    ...
    Kymera_GetApdativeAncFFGain(&gain);
    return gain;
}
...
void appTestSetAdaptiveAncGainParameters(uint32 mantissa, uint32 exponent)
{
    ...
    Kymera_SetApdativeAncGainValues(mantissa, exponent);
}
...
{
    return EarbudGamingMode_IsGamingModeEnabled();
}
...
void appTestForceAllowSleep(void)
{
    ...
    appChargerForceAllowPowerOff(TRUE);
}
                              ...
                              bool case_charger_connected)
{
    ...
    Case_PowerEvent(case_battery_state, peer_battery_state, local_battery_state, case_charger_connected);
}

bool appTestCaseEventsSupported(void)
{
    bool case_events_supported = Case_EventsEnabled();
