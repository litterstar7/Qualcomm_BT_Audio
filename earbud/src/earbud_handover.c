...
const marshal_type_descriptor_t * const  mtd_handover_app[] = {
    ...
    SINGLE_ENTITY_MARSHAL_TYPES_TABLE(EXPAND_AS_TYPE_DEFINITION)
...
static bool earbudHandover_Veto(void)
{
    ...
    if (veto)
    {
        DEBUG_LOG_INFO("earbudHandover_Veto vetoed");
    }
...
static void earbudHandover_Complete(const bool primary)
{
    ...
    if (primary)
    {
        appLinkPolicyHandoverForceUpdateHandsetLinkPolicy(&app_data->tp_bd_addr.taddr.addr);
    }
