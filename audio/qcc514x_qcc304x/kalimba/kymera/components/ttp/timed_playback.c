...
struct TIMED_PLAYBACK_STRUCT
{
    ...
    /**
     * Amount of data after processing above silence insertion threshold
     */
    unsigned latency_amount_words;
};
...
static void avoid_buffer_wrap(TIMED_PLAYBACK* timed_pb)
{
    ...
    if (output_buff_time < timed_pb->period + PERIOD_ERROR)
    {
        ...
        timed_pb->latency_amount_words = 0;
    }
    else
    {
        timed_pb->latency_amount_words =
                convert_time_to_samples(
                        time_sub(output_buff_time,
                                 timed_pb->period + PERIOD_ERROR),
                        timed_pb->sample_rate);
        return;
    }
 ...
 */
unsigned timed_playback_get_latency_amount(TIMED_PLAYBACK *timed_pb)
{
    if (timed_pb != NULL)
    {
        return timed_pb->latency_amount_words;
    }
    return 0;
}
