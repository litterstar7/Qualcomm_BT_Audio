...
{
    RINGTONE_GENERATOR_OP_DATA *rgop_data = get_instance_data(op_data);
    unsigned before_space;
    unsigned generated_words;
    unsigned before_write_offset;
    unsigned after_write_offset;
    ...
    if (rgop_data->metadata_op_buffer != NULL)
    {
        min_space = buff_metadata_available_space(rgop_data->metadata_op_buffer) / OCTETS_PER_SAMPLE;
    }
#endif /* INSTALL_METADATA */
    ...
    before_write_offset = cbuffer_get_write_offset(rgop_data->out_cbuffer);
    ...
    /* Get the movement of the output write pointer after the ringtone has run. */
    after_write_offset = cbuffer_get_write_offset(rgop_data->out_cbuffer);
    if (after_write_offset < before_write_offset)
    {
        after_write_offset += cbuffer_get_size_in_words(rgop_data->out_cbuffer);
    }
    generated_words = after_write_offset - before_write_offset;

    if (generated_words > 0)
    {
        if (rgop_data->start_seq_stage != DATA_FLOWING)
        {
