...
$cbops.multichan_sidetone_mix_op.main:
  ...
  sidetone_buffers_get_amount_loop:
   ...
   // -------------------------------------------------------------------------------
   //  Sidetone initial latency control when sidetone comes and goes:
   //
   // r0 = transfer amount from main channels, we must mix exactly this amount of
   //      sidetone samples, so some of them might be invented samples
   //
   // r6 = amount of sidetone that is available in sidetone buffers, in
   //      normal situations there are enough sidetone samples (r6 >= r0)
   //      and we just mix into main channels.
   //
   // mixing state: practically this will have impact only if ADJUST_LEVEL has been set
   //     MIXING: everything is normal and we are receiving enough sidetone to mix with
   //             main channels. If sidetone buffers underruns then will invent samples
   //             and mix after last real sidetone sample and will go to INVENTING state:
   //        main samples         |m m m m m m m m m m m m m m m m m m m m m m |
   //        real sidetone samples|s s s s s s s s s s s s s s s s             |
   //        invented samples     |                                i i i i i i |
   //
   //     INVENTING: If we receive no sidetone just silence will be mixed into main
   //                channels, if we receive some sidetone then it we make sure it
   //                is at least ADJUST_LEVEL, if not we invent further samples to
   //                start with good working latency level and then back to MIXING state:
   //        main samples         |m m m m m m m m m m m m m m m m m m m m m m |
   //        invented samples     |i i i i i                                   |
   //        real sidetone samples|          s s s s s s s s s s s s s s s s s |
   //
   // ---------------------------------------------------------------------------------

   // below:
   //   r5: mixing state, will be updated
   //   r0: transfer amount
   //   r6: amount of sidetone available
   //   r1: amount that we should invent and mix after mixing real sidetone samples
   //   r2: amount that we should invent and mix before mixing real sidetone samples
   r5 = M[r7 +  $cbops_sidetone_mix_op.multichan_st_mix_struct.MIXING_STATE_FIELD];
   if NZ jump mixing_state;

inventing_state:

   // if no real sidetone input, all will be invented, no difference between
   // pre mix or post mix (note: jumping wont change the state)
   NULL = r6;
   if Z jump mixing_state;

   // so we received some sidetone samples after entering invent state
   // all invented samples will be mixed before real sidetone
   r1 = 0;

   // Here is where we force a good working latency when sidetone resumes,
   // we make sure the sidetone level stars with at least ADJUST_LEVEL, so
   // later jitters wont cause under-run. Extra samples will be invented if needed.
   r2 = M[R7 +  $cbops_sidetone_mix_op.multichan_st_mix_struct.ADJUST_LEVEL_FIELD];
   r2 = r2 - r6;
   if LE r2 = 0;
   // no more than transfer amount
   r2 = MIN r0;
   // back to mixing state
   r5 = 1;
   jump mixing_state_done;

mixing_state:
   // in mixing state, any insertion will be post mixing
   r2 = 0;
   // see if we have enough real sidetone samples to mix
   r1 = r0 - r6;
   if NEG r1 = 0;
   // go to inventing state if underruns
   if NZ r5 = 0;
mixing_state_done:
   // r5: new mixing state
   M[r7 +  $cbops_sidetone_mix_op.multichan_st_mix_struct.MIXING_STATE_FIELD] = r5;

   // M3: Amount inventing samples will be done this time
   M3 = r2 + r1;  // only one of r1 or r2 can be non-0

   // I5: amount of inventing samples to be mixed before real samples
   I5 = r2;
   ...
   transfer_and_sidetone_mix_channel_loop:
      ...
      // I5  of M3 this amount will be written before real mix
      // I1/L1/B1 = Sidetone buffer
      // I0/L0/B0 = Input buffer
      // I4/L4/B4 = Output buffer

      // pre-mix invent samples
      r10 = I5;
      if Z jump pre_mix_invented_sidetone_done;
      // Repeat first sample of sidetone buffer (insert sidetone samples)
      r0 = M[I1,M1];
      r0 = M[I1,-MK1];
      // Ensure sidetone is connected repeat first sample, else mix silence
      NULL = r6;
      if Z r0=NULL;
      do pre_mix_invented_sidetone_loop;
         // calculate the current output sample
         rMAC = rMAC + r0 * r5;
         // get the next input value and write the result
         rMAC = M[I0,M1], M[I4,M1] = rMAC;
      pre_mix_invented_sidetone_loop:
      pre_mix_invented_sidetone_done:
      ...
      NULL = r6;
      if Z r0=NULL;
      do post_mix_invented_sidetone_loop;
         // calculate the current output sample
         rMAC = rMAC + r0 * r5;
         // get the next input value and write the result
         rMAC = M[I0,M1], M[I4,M1] = rMAC;
      post_mix_invented_sidetone_loop:
